...
using RTSafe.DxfCore.Header;
using RTSafe.DxfCore.Tables;
using Attribute = RTSafe.DxfCore.Entities.Attribute;
...
namespace RTSafe.DxfCore
{
    ...
    public sealed class DxfReader
    {
        ...
        private List<IEntityObject> shaps;
        ...
        public DxfCore.Model.Point ExtMaxPoint { get { return extMaxPoint; } set { this.extMaxPoint = value; } }

        private DxfCore.Model.Point extMaxPoint = new DxfCore.Model.Point();

        public DxfCore.Model.Point ExtMinPoint { get { return extMinPoint; } set { this.extMinPoint = value; } }
        private DxfCore.Model.Point extMinPoint = new DxfCore.Model.Point();

        #endregion

        #region public entity properties

        public List<IEntityObject> Shaps
        {
            get { return this.shaps; }
        }
        ...
        public void Open()
        {
            try
            {
                ...
                this.reader = new StreamReader(this.input, Encoding.UTF8);
                ...
                if (HeaderVariable.Allowed.ContainsKey(code.Value))
                {
                    ...
                    switch (variableName)
                    {
                        ...
                        case SystemVariable.HandSeed:
                            ...
                            break;
                        case SystemVariable.ExtMax:
                            ReadExtMax(ref code);
                            break;
                        case SystemVariable.ExtMin:
                            ReadExtMin(ref code);
                            break;
        ...
        private void ReadExtMax(ref CodeValuePair code)
        {
            if (code.Code == 10)
            {
                this.extMaxPoint.X = double.Parse(code.Value);
                code = this.ReadCodePair();
            }
            if (code.Code == 20)
            {
                extMaxPoint.Y = double.Parse(code.Value);
            }
        }

        private void ReadExtMin(ref CodeValuePair code)
        {

            if (code.Code == 10)
            {
                this.extMinPoint.X = double.Parse(code.Value);
                code = this.ReadCodePair();
            }
            if (code.Code == 20)
            {
                extMinPoint.Y = double.Parse(code.Value);
            }
        ...
        private void ReadApplicationsId()
        {
            ...
            while (code.Value != StringCode.EndTable)
            {
                if (code.Value == StringCode.ApplicationIDTable)
                {
                    ...
                    if (!appIds.ContainsKey(appId.Name))
                ...
                switch (code.Code)
                {
                    ...
                    case 0: // entity
                        ...
                        if (entity != null)
                            ...
                            {
                                if (!attdefs.ContainsKey(((AttributeDefinition)entity).Id))
            ...
            while (code.Code != 0)
            {
                switch (code.Code)
                {
                    ...
                    case 2:
                        ...
                        {
                            attdef = new AttributeDefinition(Guid.NewGuid().ToString());
                        }
                        break;
                    case 1:
                        value = code.Value;
            ...
            while (code.Value != StringCode.EndSection)
            {
                ...
                switch (code.Value)
                {
                    ...
                    case DxfObjectCode.Hatch:
                        entity = this.ReadHatch(ref code);
                        break;
                    case DxfObjectCode.Attdef:
                        entity = this.ReadAttdef(ref code); break;
                    default:
                        ReadUnknowEntity(ref code);
                        //code = this.ReadCodePair();
                        break;
                ...
                if (entity != null)
                {
                    this.shaps.Add(entity);
                }
            }
        }

        private IEntityObject ReadAttdef(ref CodeValuePair code)
        {
            var text = new Text();
            ...
            text.Value = string.IsNullOrEmpty(value) ? text.Value : value;
            TextAlignment alignment = ObtainAlignment(horizontalAlignment, verticalAlignment);

            text.BasePoint = (horizontalAlignment == 0 || verticalAlignment == 0) ? firstAlignmentPoint : secondAlignmentPoint;
            text.Normal = normal;
        ...
        private Hatch ReadHatch(ref CodeValuePair code)
        {
            var hatch = new Hatch();
            ...
            while (code.Code != 0)
            {
                ...
                {
                    case 5:
                        hatch.Handle = code.Value;
                        code = this.ReadCodePair();
                        break;
                    case 8: //layer code
                        hatch.Layer = this.GetLayer(code.Value);
                        code = this.ReadCodePair();
                        break;
                    case 62:
                        hatch.Color = new AciColor(short.Parse(code.Value));
                        code = this.ReadCodePair();
                        break;
                    case 63:
                        hatch.Color = new AciColor(short.Parse(code.Value));
                        ...
                        break;
                    case 6: //type line code
                        hatch.LineType = this.GetLineType(code.Value);
                        code = this.ReadCodePair();
                        break;
                    case 10:
                        basePoint.X = double.Parse(code.Value);
        ...
        private IEntityObject ReadBoundaryPathData(ref CodeValuePair code)
        {
            if (code.Value.Trim() == "1")
            {
                var entity = new Line();
                var startPoint = Vector3f.Zero;
                var endPoint = Vector3f.Zero;
                code = this.ReadCodePair();
                while (code.Code == 10 || code.Code == 20 || code.Code == 11 || code.Code == 21)
                {
                    switch (code.Code)
                    {
                        case 10: startPoint.X = double.Parse(code.Value); break;

                        case 20: startPoint.Y = double.Parse(code.Value); break;

                        case 11: endPoint.X = double.Parse(code.Value); break;

                        case 21: endPoint.Y = double.Parse(code.Value); break;
                    }
                    code = this.ReadCodePair();
                }
            ...
            if (code.Value.Trim() == "2")
            {
                Vector3f center = Vector3f.Zero;
                double radius = 0.0d;
                double startAngle = 0.0d;
                double endAngle = 0.0d;
                bool flag = false;
                code = this.ReadCodePair();
                while (code.Code == 10 || code.Code == 20 ||
                    code.Code == 40 || code.Code == 50 || code.Code == 51 || code.Code == 73)
                {
                    switch (code.Code)
                    {
                        case 10: center.X = double.Parse(code.Value); break;

                        case 20: center.Y = double.Parse(code.Value); break;

                        case 40: radius = double.Parse(code.Value); break;

                        case 50: startAngle = double.Parse(code.Value); break;
                        case 51: endAngle = double.Parse(code.Value); break;
                        case 73: flag = bool.Parse(code.Value); break;
                    }
                    code = this.ReadCodePair();
                }
                var entity = new Arc(center, radius, startAngle, endAngle);
                return entity;
            }
            if (code.Value.Trim() == "3")
            {
                Vector3f center = Vector3f.Zero;
                double radius = 0.0d;
                double startAngle = 0.0d;
                    ...
                    code.Code == 40 || code.Code == 50 || code.Code == 51 || code.Code == 73)
                {
                    switch (code.Code)
                    {
                        ...
                        case 50: startAngle = double.Parse(code.Value); break;
                        case 51: endAngle = double.Parse(code.Value); break;
                        case 73: flag = code.Value.Trim().Equals(1); break;
                    }
                    code = this.ReadCodePair();
                }
                var entity = new Arc(center, radius, startAngle, endAngle);
        ...
        private void ReadLayers()
        {
            ...
            while (code.Value != StringCode.EndTable)
            {
                if (code.Value == StringCode.LayerTable)
                {
                    ...
                    if (!layers.ContainsKey(capa.Name))
        ...
        private void ReadLineTypes()
        {
            ...
            while (code.Value != StringCode.EndTable)
            {
                if (code.Value == StringCode.LineTypeTable)
                {
                    ...
                    if (!this.lineTypes.ContainsKey(tl.Name))
        ...
        private void ReadTextStyles()
        {
            ...
            while (code.Value != StringCode.EndTable)
            {
                if (code.Value == StringCode.TextStyleTable)
                {
                    ...
                    if (!string.IsNullOrEmpty(style.Name) && !this.textStyles.ContainsKey(style.Name))
            ...
            while (code.Code != 0)
            {
                switch (code.Code)
                {
                    ...
                    case 2:
                        ...
                        //                                                 "Invalid value " + code.Value + " in code " + code.Code + " line " + this.fileLine);
                        //}
                        name = string.IsNullOrEmpty(code.Value) ? Guid.NewGuid().ToString() : code.Value;
                        break;
                    case 3:
                        //if (string.IsNullOrEmpty(code.Value))
                        //{
            ...
            while (code.Code != 0)
            {
                switch (code.Code)
                {
                    ...
                    case 30:
                        ...
                        break;
                    case 40:
                        arc.Radius = (int) double.Parse(code.Value);
                        code = this.ReadCodePair();
                        break;
                    case 50:
        ...
        private Ellipse ReadEllipse(ref CodeValuePair code)
        {
            ...
            double rotation = (double)Vector2d.AngleBetween(Vector2d.UnitX, new Vector2d(ocsAxisPoint.X, ocsAxisPoint.Y));
        ...
        private Text ReadText(ref CodeValuePair code)
        {
            ...
            text.BasePoint = firstAlignmentPoint;
            text.Normal = normal;
            text.Alignment = alignment;
            text.XData = xData;
            return text;
        ...
        private Block GetBlock(string name)
        {
            ...
            {
                this.blocks.Add(name, new Block(name));
            }
        ...
        private XData ReadXDataRecord(string appId, ref CodeValuePair code)
        {
            ...
            {
                appIds.Add(appId, new ApplicationRegistry(appId));
            }
            xData = new XData(this.appIds[appId]);
            code = this.ReadCodePair();
